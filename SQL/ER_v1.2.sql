...
( 
	...
	[ActivationTime]     char(18)  NOT NULL ,
...
BEGIN
  DECLARE  @errno   int,
           ...
           @errmsg  varchar(255)
    /* erwin Builtin Trigger */
    /* Operator  ALARM on child delete no action */
    /* ERWIN_RELATION:CHECKSUM="0002b583", PARENT_OWNER="", PARENT_TABLE="Operator"
    CHILD_OWNER="", CHILD_TABLE="ALARM"
    P2C_VERB_PHRASE="", C2P_VERB_PHRASE="", 
    FK_CONSTRAINT="R_11", FK_COLUMNS="AcknowledgedBy" */
    IF EXISTS (SELECT * FROM deleted,Operator
      WHERE
        /* %JoinFKPK(deleted,Operator," = "," AND") */
        deleted.AcknowledgedBy = Operator.OperatorId AND
        NOT EXISTS (
          SELECT * FROM ALARM
          WHERE
            /* %JoinFKPK(ALARM,Operator," = "," AND") */
            ALARM.AcknowledgedBy = Operator.OperatorId
        )
    )
    BEGIN
      SELECT @errno  = 30010,
             @errmsg = 'Cannot delete last ALARM because Operator exists.'
      GOTO error
    END
...
BEGIN
  ...
  /* erwin Builtin Trigger */
  /* Operator  ALARM on child update no action */
  /* ERWIN_RELATION:CHECKSUM="00031905", PARENT_OWNER="", PARENT_TABLE="Operator"
    CHILD_OWNER="", CHILD_TABLE="ALARM"
    P2C_VERB_PHRASE="", C2P_VERB_PHRASE="", 
    FK_CONSTRAINT="R_11", FK_COLUMNS="AcknowledgedBy" */
  IF
  ...
  BEGIN
    ...
    SELECT @validcnt = count(*)
      FROM inserted,Operator
        WHERE
          /* %JoinFKPK(inserted,Operator) */
          inserted.AcknowledgedBy = Operator.OperatorId
    ...
    BEGIN
      SELECT @errno  = 30007,
             @errmsg = 'Cannot update ALARM because Operator does not exist.'
      GOTO error
    END
  END
...
BEGIN
  DECLARE  @errno   int,
    ...
    /* ERWIN_RELATION:CHECKSUM="0004dea9", PARENT_OWNER="", PARENT_TABLE="ALARMCONFIGURATION"
    CHILD_OWNER="", CHILD_TABLE="ALARM"
    P2C_VERB_PHRASE="", C2P_VERB_PHRASE="", 
    FK_CONSTRAINT="R_9", FK_COLUMNS="TagId""AlarmId" */
    IF EXISTS (
    ...
    /* erwin Builtin Trigger */
    /* PRIORITYLIST  ALARMCONFIGURATION on child delete no action */
    /* ERWIN_RELATION:CHECKSUM="00000000", PARENT_OWNER="", PARENT_TABLE="PRIORITYLIST"
    CHILD_OWNER="", CHILD_TABLE="ALARMCONFIGURATION"
    P2C_VERB_PHRASE="", C2P_VERB_PHRASE="", 
    FK_CONSTRAINT="R_17", FK_COLUMNS="PriorityLevel" */
    IF EXISTS (SELECT * FROM deleted,PRIORITYLIST
      WHERE
        /* %JoinFKPK(deleted,PRIORITYLIST," = "," AND") */
        deleted.PriorityLevel = PRIORITYLIST.PriorityLevel AND
        NOT EXISTS (
          SELECT * FROM ALARMCONFIGURATION
          WHERE
            /* %JoinFKPK(ALARMCONFIGURATION,PRIORITYLIST," = "," AND") */
            ALARMCONFIGURATION.PriorityLevel = PRIORITYLIST.PriorityLevel
        )
    )
    BEGIN
      SELECT @errno  = 30010,
             @errmsg = 'Cannot delete last ALARMCONFIGURATION because PRIORITYLIST exists.'
      GOTO error
    END
...
BEGIN
  ...
  /* ERWIN_RELATION:CHECKSUM="0005479c", PARENT_OWNER="", PARENT_TABLE="ALARMCONFIGURATION"
    CHILD_OWNER="", CHILD_TABLE="ALARM"
    P2C_VERB_PHRASE="", C2P_VERB_PHRASE="", 
    FK_CONSTRAINT="R_9", FK_COLUMNS="TagId""AlarmId" */
  IF
  ...
  /* erwin Builtin Trigger */
  /* PRIORITYLIST  ALARMCONFIGURATION on child update no action */
  /* ERWIN_RELATION:CHECKSUM="00000000", PARENT_OWNER="", PARENT_TABLE="PRIORITYLIST"
    CHILD_OWNER="", CHILD_TABLE="ALARMCONFIGURATION"
    P2C_VERB_PHRASE="", C2P_VERB_PHRASE="", 
    FK_CONSTRAINT="R_17", FK_COLUMNS="PriorityLevel" */
  ...
  BEGIN
    SELECT @nullcnt = 0
    SELECT @validcnt = count(*)
      FROM inserted,PRIORITYLIST
        WHERE
          /* %JoinFKPK(inserted,PRIORITYLIST) */
          inserted.PriorityLevel = PRIORITYLIST.PriorityLevel
    /* %NotnullFK(inserted," IS NULL","select @nullcnt = count(*) from inserted where"," AND") */
    select @nullcnt = count(*) from inserted where
      inserted.PriorityLevel IS NULL
    IF @validcnt + @nullcnt != @numrows
    BEGIN
      SELECT @errno  = 30007,
             @errmsg = 'Cannot update ALARMCONFIGURATION because PRIORITYLIST does not exist.'
      GOTO error
    END
  END
...
CREATE TRIGGER tD_Operator ON Operator FOR DELETE AS
/* erwin Builtin Trigger */
...
BEGIN
  DECLARE  @errno   int,
           ...
           @errmsg  varchar(255)
    /* erwin Builtin Trigger */
    /* Operator  ALARM on parent delete no action */
    /* ERWIN_RELATION:CHECKSUM="00010c20", PARENT_OWNER="", PARENT_TABLE="Operator"
    CHILD_OWNER="", CHILD_TABLE="ALARM"
    P2C_VERB_PHRASE="", C2P_VERB_PHRASE="", 
    FK_CONSTRAINT="R_11", FK_COLUMNS="AcknowledgedBy" */
    IF EXISTS (
      ...
      WHERE
        /*  %JoinFKPK(ALARM,deleted," = "," AND") */
        ALARM.AcknowledgedBy = deleted.OperatorId
    )
    BEGIN
      SELECT @errno  = 30001,
             @errmsg = 'Cannot delete Operator because ALARM exists.'
      GOTO error
    END
...
CREATE TRIGGER tU_Operator ON Operator FOR UPDATE AS
/* erwin Builtin Trigger */
/* UPDATE trigger on Operator */
BEGIN
  DECLARE  @numrows int,
           @nullcnt int,
           @validcnt int,
           @insOperatorId integer,
           @errno   int,
           @severity int,
           @state    int,
           @errmsg  varchar(255)
  ...
  SELECT @numrows = @@rowcount
  /* erwin Builtin Trigger */
  /* Operator  ALARM on parent update no action */
  /* ERWIN_RELATION:CHECKSUM="000133ca", PARENT_OWNER="", PARENT_TABLE="Operator"
    CHILD_OWNER="", CHILD_TABLE="ALARM"
    P2C_VERB_PHRASE="", C2P_VERB_PHRASE="", 
    FK_CONSTRAINT="R_11", FK_COLUMNS="AcknowledgedBy" */
  IF
  ...
  BEGIN
    ...
    BEGIN
      SELECT @errno  = 30005,
             @errmsg = 'Cannot update Operator because ALARM exists.'
      GOTO error
    END
  END
...
CREATE TRIGGER tD_PRIORITYLIST ON PRIORITYLIST FOR DELETE AS
/* erwin Builtin Trigger */
/* DELETE trigger on PRIORITYLIST */
BEGIN
  DECLARE  @errno   int,
           @severity int,
           @state    int,
           @errmsg  varchar(255)
    /* erwin Builtin Trigger */
    /* PRIORITYLIST  ALARMCONFIGURATION on parent delete no action */
    /* ERWIN_RELATION:CHECKSUM="0001380c", PARENT_OWNER="", PARENT_TABLE="PRIORITYLIST"
    CHILD_OWNER="", CHILD_TABLE="ALARMCONFIGURATION"
    P2C_VERB_PHRASE="", C2P_VERB_PHRASE="", 
    FK_CONSTRAINT="R_17", FK_COLUMNS="PriorityLevel" */
    IF EXISTS (
      ...
      WHERE
        /*  %JoinFKPK(ALARMCONFIGURATION,deleted," = "," AND") */
        ALARMCONFIGURATION.PriorityLevel = deleted.PriorityLevel
    )
    BEGIN
      SELECT @errno  = 30001,
             @errmsg = 'Cannot delete PRIORITYLIST because ALARMCONFIGURATION exists.'
      GOTO error
    END
...
CREATE TRIGGER tU_PRIORITYLIST ON PRIORITYLIST FOR UPDATE AS
/* erwin Builtin Trigger */
/* UPDATE trigger on PRIORITYLIST */
BEGIN
  DECLARE  @numrows int,
           @nullcnt int,
           @validcnt int,
           @insPriorityLevel char(20),
           @errno   int,
           @severity int,
           @state    int,
           @errmsg  varchar(255)
  ...
  SELECT @numrows = @@rowcount
  /* erwin Builtin Trigger */
  /* PRIORITYLIST  ALARMCONFIGURATION on parent update no action */
  /* ERWIN_RELATION:CHECKSUM="00014d25", PARENT_OWNER="", PARENT_TABLE="PRIORITYLIST"
    CHILD_OWNER="", CHILD_TABLE="ALARMCONFIGURATION"
    P2C_VERB_PHRASE="", C2P_VERB_PHRASE="", 
    FK_CONSTRAINT="R_17", FK_COLUMNS="PriorityLevel" */
  IF
  ...
  BEGIN
    ...
    BEGIN
      SELECT @errno  = 30005,
             @errmsg = 'Cannot update PRIORITYLIST because ALARMCONFIGURATION exists.'
      GOTO error
    END
  END
