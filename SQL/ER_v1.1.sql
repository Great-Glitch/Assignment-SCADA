...
ALTER TABLE [Priority]
	ADD CONSTRAINT [XPKPriority] PRIMARY KEY  CLUSTERED ([Priority] ASC)
go
...
ALTER TABLE [ALARMCONFIGURATION]
	ADD CONSTRAINT [R_17] FOREIGN KEY ([Priority]) REFERENCES [Priority]([Priority])
		ON DELETE NO ACTION
		ON UPDATE NO ACTION
go
...
BEGIN
  DECLARE  @errno   int,
    ...
    /* ERWIN_RELATION:CHECKSUM="0004c0d3", PARENT_OWNER="", PARENT_TABLE="ALARMCONFIGURATION"
    CHILD_OWNER="", CHILD_TABLE="ALARM"
    P2C_VERB_PHRASE="", C2P_VERB_PHRASE="", 
    FK_CONSTRAINT="R_9", FK_COLUMNS="TagId""AlarmId" */
    IF EXISTS (
    ...
    /* erwin Builtin Trigger */
    /* Priority  ALARMCONFIGURATION on child delete no action */
    /* ERWIN_RELATION:CHECKSUM="00000000", PARENT_OWNER="", PARENT_TABLE="Priority"
    CHILD_OWNER="", CHILD_TABLE="ALARMCONFIGURATION"
    P2C_VERB_PHRASE="", C2P_VERB_PHRASE="", 
    FK_CONSTRAINT="R_17", FK_COLUMNS="Priority" */
    IF EXISTS (SELECT * FROM deleted,Priority
      WHERE
        /* %JoinFKPK(deleted,Priority," = "," AND") */
        deleted.Priority = Priority.Priority AND
        NOT EXISTS (
          SELECT * FROM ALARMCONFIGURATION
          WHERE
            /* %JoinFKPK(ALARMCONFIGURATION,Priority," = "," AND") */
            ALARMCONFIGURATION.Priority = Priority.Priority
        )
    )
    BEGIN
      SELECT @errno  = 30010,
             @errmsg = 'Cannot delete last ALARMCONFIGURATION because Priority exists.'
      GOTO error
    END
...
BEGIN
  ...
  /* ERWIN_RELATION:CHECKSUM="00053843", PARENT_OWNER="", PARENT_TABLE="ALARMCONFIGURATION"
    CHILD_OWNER="", CHILD_TABLE="ALARM"
    P2C_VERB_PHRASE="", C2P_VERB_PHRASE="", 
    FK_CONSTRAINT="R_9", FK_COLUMNS="TagId""AlarmId" */
  IF
  ...
  /* erwin Builtin Trigger */
  /* Priority  ALARMCONFIGURATION on child update no action */
  /* ERWIN_RELATION:CHECKSUM="00000000", PARENT_OWNER="", PARENT_TABLE="Priority"
    CHILD_OWNER="", CHILD_TABLE="ALARMCONFIGURATION"
    P2C_VERB_PHRASE="", C2P_VERB_PHRASE="", 
    FK_CONSTRAINT="R_17", FK_COLUMNS="Priority" */
  ...
  BEGIN
    SELECT @nullcnt = 0
    SELECT @validcnt = count(*)
      FROM inserted,Priority
        WHERE
          /* %JoinFKPK(inserted,Priority) */
          inserted.Priority = Priority.Priority
    /* %NotnullFK(inserted," IS NULL","select @nullcnt = count(*) from inserted where"," AND") */
    select @nullcnt = count(*) from inserted where
      inserted.Priority IS NULL
    IF @validcnt + @nullcnt != @numrows
    BEGIN
      SELECT @errno  = 30007,
             @errmsg = 'Cannot update ALARMCONFIGURATION because Priority does not exist.'
      GOTO error
    END
  END
...
CREATE TRIGGER tD_Priority ON Priority FOR DELETE AS
/* erwin Builtin Trigger */
/* DELETE trigger on Priority */
BEGIN
  DECLARE  @errno   int,
           @severity int,
           @state    int,
           @errmsg  varchar(255)
    /* erwin Builtin Trigger */
    /* Priority  ALARMCONFIGURATION on parent delete no action */
    /* ERWIN_RELATION:CHECKSUM="00012f13", PARENT_OWNER="", PARENT_TABLE="Priority"
    CHILD_OWNER="", CHILD_TABLE="ALARMCONFIGURATION"
    P2C_VERB_PHRASE="", C2P_VERB_PHRASE="", 
    FK_CONSTRAINT="R_17", FK_COLUMNS="Priority" */
    IF EXISTS (
      ...
      WHERE
        /*  %JoinFKPK(ALARMCONFIGURATION,deleted," = "," AND") */
        ALARMCONFIGURATION.Priority = deleted.Priority
    )
    BEGIN
      SELECT @errno  = 30001,
             @errmsg = 'Cannot delete Priority because ALARMCONFIGURATION exists.'
      GOTO error
    END
...
CREATE TRIGGER tU_Priority ON Priority FOR UPDATE AS
/* erwin Builtin Trigger */
/* UPDATE trigger on Priority */
BEGIN
  DECLARE  @numrows int,
           @nullcnt int,
           @validcnt int,
  ...
  SELECT @numrows = @@rowcount
  /* erwin Builtin Trigger */
  /* Priority  ALARMCONFIGURATION on parent update no action */
  /* ERWIN_RELATION:CHECKSUM="00014091", PARENT_OWNER="", PARENT_TABLE="Priority"
    CHILD_OWNER="", CHILD_TABLE="ALARMCONFIGURATION"
    P2C_VERB_PHRASE="", C2P_VERB_PHRASE="", 
    FK_CONSTRAINT="R_17", FK_COLUMNS="Priority" */
  IF
  ...
  BEGIN
    ...
    BEGIN
      SELECT @errno  = 30005,
             @errmsg = 'Cannot update Priority because ALARMCONFIGURATION exists.'
      GOTO error
    END
  END
